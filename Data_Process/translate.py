...
# 读取CSV文件
df = pd.read_csv("datasets_museum/asian_tt.csv")
...
for index, row in df.iterrows():
    ...
    for column in df.columns:
        if column == 'Img_url' or column == 'Img_path':
            continue
        content = row[column]
        if pd.isna(content):
            continue
        translated_content = translate(str(content), "auto", "zh")
...
# 保存翻译后的CSV文件
df_translated.to_csv("datasets_museum/asian_trans.csv", index=False)
